...
2026-05-21 - PyPI naming collision: `youtube-mcp` and `youtube-mcp-server` are already occupied on PyPI by unrelated packages. This project should publish as `youtube-api-mcp` and keep `youtube-mcp` only as a console-script alias. End-user uvx docs should prefer `uvx youtube-api-mcp ...`; alias-only usage requires `uvx --from youtube-api-mcp youtube-mcp ...`.
